...
OUTPUT_FILE=./ubuntu-cloudinit.img
...
mcopy -oi "$OUTPUT_FILE" -s local/user-data ::
mcopy -oi "$OUTPUT_FILE" -s local/meta-data ::
mcopy -oi "$OUTPUT_FILE" -s local/network-config ::
